...
from app.frontend.models.entities import User
...
class AppController:
    ...
    def refresh_product_list(self) -> None:
        ...
        if not cards:
            ...
            cards = [card(muted_text("No se encontraron productos con ese filtro."), padding=20)]
    ...
    async def refresh_cart_quote(self) -> None:
        ...
        if not self.state.current_user or not self.state.cart:
            self.page.update()
            return
        ...
        try:
            ...
            self.cart_quote_box.controls = [
                self.quote_row("Subtotal", currency(int(self.cart_quote.get("subtotal", 0)))),
                self.quote_row("Descuento preferencias", f"-{currency(int(self.cart_quote.get('preference_discount', 0)))}"),
                self.quote_row("Descuento puntos", f"-{currency(int(self.cart_quote.get('points_discount', 0)))}"),
                ft.Text(
                    self.cart_quote.get("points_discount_label", ""),
                    color=IschuuColors.TEXT_MUTED,
                    size=12,
                ),
                self.quote_row("Total Webpay", currency(int(self.cart_quote.get("total", 0))), True),
            ]
        except Exception as exc:
            self.show_message(f"No se pudo calcular descuento: {exc}", error=True)
    ...
    async def handle_login(self, email: str, password: str) -> None:
        try:
            ...
            print("USUARIO LOGIN:", user_data)
            print("ES ADMIN:", user_data.get("is_admin"))
    ...
    def handle_change_quantity(self, product_id: str, delta: int) -> None:
        item = next((cart_item for cart_item in self.state.cart if cart_item.product.id == product_id), None)
        if not item:
            return
        ...
        if new_quantity <= 0:
            self.state.cart = [
                cart_item for cart_item in self.state.cart
    ...
    async def handle_checkout(self) -> None:
        ...
        try:
            ...
            self.show_message("Pago iniciado. Luego de pagar, vuelve y presiona Verificar pago.")
        except Exception as exc:
