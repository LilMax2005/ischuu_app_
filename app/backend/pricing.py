...
def calculate_points_discount(
    ...
    if not use_points or user_points < MIN_POINTS_TO_REDEEM:
        return {
            "points_to_spend": 0,
            "points_discount": 0,
    ...
    if requested_points is not None:
        max_usable_points = min(max_usable_points, max(0, int(requested_points)))

    if max_usable_points < MIN_POINTS_TO_REDEEM:
        return {
            "points_to_spend": 0,
            "points_discount": 0,
            "points_discount_label": "Necesitas al menos 100 puntos para canjear",
        }
...
def earned_points_for_amount(product_amount_paid: int) -> int:
    """
    1 punto por cada $1.000 pagados en productos.
    El envío no genera puntos.
    """
    if product_amount_paid <= 0:
        return 0

    return product_amount_paid // 1000
